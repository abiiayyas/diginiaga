...
<div class="flex flex-col h-full bg-gray-100 rounded-2xl overflow-hidden border">
    <div class="flex items-center justify-between px-4 py-3 border-b bg-white">
        <h3 class="text-sm font-semibold text-gray-700">Preview Landing Page</h3>
        <span class="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-1 rounded-md">Mobile View</span>
    </div>
    <div class="flex-1 p-4 md:p-8 overflow-y-auto flex items-start justify-center">
        <div class="w-full max-w-[380px] bg-white rounded-[2rem] shadow-2xl overflow-hidden border-4 border-gray-800 relative" style="font-family: 'Inter', sans-serif; line-height: 1.5; min-height: 700px;">
            
            {{-- Notch & Status bar mock --}}
            <div class="bg-white text-gray-800 text-[10px] flex justify-between items-center px-6 py-2">
            ...
            <div class="px-5 pb-8 overflow-y-auto h-[calc(100%-24px)]">
                ...
                <div class="text-center pt-6 pb-5">
                    ...
                    <h1 class="text-xl font-extrabold leading-tight text-gray-900" x-text="headline || 'Headline Produk Utama'"></h1>
                    <p class="text-sm text-gray-500 mt-2" x-text="subheadline || 'Subheadline deskripsi singkat yang mendukung.'"></p>
                </div>
                ...
                <template x-if="body_content">
                    <div class="mb-6 text-left">
                        <h2 class="text-sm font-bold text-gray-800 mb-2 border-b pb-1">Detail Produk</h2>
                        <div class="text-xs text-gray-600 whitespace-pre-wrap leading-relaxed" x-text="body_content"></div>
                    </div>
                </template>
                        ...
                        <template x-if="sliderPreviews.length > 1">
                            <div class="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 bg-black/30 px-2 py-1.5 rounded-full backdrop-blur-sm">
                                <template x-for="(s, si) in sliderPreviews" :key="si">
                                    <span class="w-1.5 h-1.5 rounded-full transition-all" :class="si === sliderIdx ? 'bg-white scale-110' : 'bg-white/50'"></span>
                                </template>
                            </div>
                        </template>
                    </div>
                ...
                <template x-if="faqs.filter(f => f.q && f.a).length">
                    <div class="mb-6">
                        <h2 class="text-sm font-bold text-gray-800 text-center mb-4">Pertanyaan Umum</h2>
                        <div class="space-y-2">
                            <template x-for="(f, fi) in faqs.filter(f => f.q && f.a)" :key="fi">
                                <div class="bg-gray-50 rounded-xl overflow-hidden transition-all border border-gray-100">
                                    <div class="flex justify-between items-center px-4 py-3 text-xs font-semibold text-gray-700 cursor-pointer" @click="toggleFaq(fi)">
                                        <span x-text="f.q"></span>
                                        <span class="text-[10px] text-gray-400 transition-transform" :class="fi === 0 ? 'rotate-180' : ''">▼</span>
                                    </div>
                                    <div class="px-4 pb-3 text-xs text-gray-500 leading-relaxed bg-white" x-show="fi === 0 || false" x-text="f.a"></div>
                                </div>
                            </template>
                        </div>
                    </div>
                ...
                <template x-if="testiItems().length">
                    <div class="mb-6">
                        <h2 class="text-sm font-bold text-gray-800 text-center mb-4">Apa Kata Mereka</h2>
                        <div class="space-y-3">
                            <template x-for="t in testiItems()" :key="t.name">
                                <div class="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
                                    <div class="flex text-yellow-400 text-xs mb-2">★★★★★</div>
                                    <div class="text-xs text-gray-700 mb-3 leading-relaxed" x-text="'\"'+t.quote+'\"'"></div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div>
                                            <div class="text-[11px] font-bold text-gray-900" x-text="t.name"></div>
                                            <div class="text-[10px] text-gray-400" x-text="t.role"></div>
                                        </div>
                                    </div>
                                </div>
                            </template>
                ...
                <div class="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5 text-center mt-8 shadow-xl">
                    ...
                    <template x-if="button_text && button_url">
                        <div class="mt-3 w-full py-3 rounded-xl text-white text-xs font-semibold border border-gray-600 bg-white/5 backdrop-blur-sm"
                             x-text="button_text"></div>
                    </template>
                </div>

                {{-- Footer --}}
                <div class="text-center text-[10px] text-gray-400 pt-6 pb-2">
