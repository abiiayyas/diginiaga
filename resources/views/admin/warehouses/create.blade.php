...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Tambah Gudang Baru</h1>
        <a href="{{ route('admin.warehouses.index') }}" class="text-gray-600 hover:text-gray-900 font-medium text-sm">Kembali</a>
    </div>

    <form action="{{ route('admin.warehouses.store') }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        @csrf
        <div class="p-6 md:p-8 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required
                       class="w-full rounded-xl border-gray-200 focus:border-brand-500 focus:ring focus:ring-brand-500 focus:ring-opacity-50"
            ...
            <div class="relative" x-data="areaSearch()">
                <label for="area_search" class="block text-sm font-medium text-gray-700 mb-1">Pilih Area / Kota *</label>
                ...
                <div x-show="loading" class="absolute right-3 top-9 text-gray-400">
                ...
                <!-- Dropdown -->
                <div x-show="isOpen && results.length > 0" class="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-auto">
                    <ul class="py-1">
                        <template x-for="area in results" :key="area.id">
                            <li @click="selectArea(area)" class="px-4 py-2 hover:bg-brand-50 cursor-pointer">
                                <div class="text-sm font-medium text-gray-900" x-text="area.name"></div>
                                <div class="text-xs text-gray-500" x-text="area.administrative_division_level_2_name + ', ' + area.administrative_division_level_1_name"></div>
                            </li>
                        </template>
                    </ul>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="province" id="province" value="{{ old('province') }}" readonly
                           class="w-full rounded-xl border-gray-200 bg-gray-50 text-gray-500">
                </div>
                <div>
                    <label for="postal_code" class="block text-sm font-medium text-gray-700 mb-1">Kode Pos</label>
                    <input type="text" name="postal_code" id="postal_code" value="{{ old('postal_code') }}" readonly
                           class="w-full rounded-xl border-gray-200 bg-gray-50 text-gray-500">
                </div>
                <div>
                    <label for="mengantar_area_id" class="block text-sm font-medium text-gray-700 mb-1">Mengantar Area ID</label>
                    <input type="text" name="mengantar_area_id" id="mengantar_area_id" value="{{ old('mengantar_area_id') }}" readonly
                           class="w-full rounded-xl border-gray-200 bg-gray-50 text-gray-500">
                </div>
            </div>
            ...
            <!-- Alamat Lengkap -->
            <div>
                <label for="address" class="block text-sm font-medium text-gray-700 mb-1">Alamat Lengkap Jalan *</label>
                <textarea name="address" id="address" rows="3" required
                          class="w-full rounded-xl border-gray-200 focus:border-brand-500 focus:ring focus:ring-brand-500 focus:ring-opacity-50"
                          placeholder="Nama jalan, nomor gedung, dsb.">{{ old('address') }}</textarea>
